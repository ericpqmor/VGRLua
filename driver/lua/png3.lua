...
function horizontal_rational_quadratic_test(x0,y0,x1,y1,w1,x2,y2,x,y,coefs,xmin,ymin,xmax,ymax,diagonal)
	...
	if ymin <= y and y < ymax and x <= xmax then
		...
		--Inside the bounding box. Diagonal test
			if diagonal == true then
				if horizontal_test_linear_segment(x0,y0,x2,y2,x,y) == true then
					test = horizontal_implicit_rational_quadratic_test(x0,y0,x1,y1,w1,x2,y2,sign,x,y)
				else
					test = false
				end
			else
...
local function WindingIncrement(tree, ind, shape, segment_num, k)
  ...
  if instruction == 'rational_quadratic_segment' then
    ...
    if bymin < ymax and bymin >= ymin then
      ...
      if horizontal_implicit_rational_quadratic_test(x0,y0,x1,y1,w1,x2,y2,sign,bxmax,bymin) then return util.sign(y2-y0) end
    end
  end
...
function insidetest_rationalquadratic(x0,y0,x1,y1,w1,x2,y2,xmin,ymin,xmax,ymax)
  ...
  if (xmin<x0 and x0<xmax and ymin<y0 and y0<ymax) or (xmin<x2 and x2<xmax and ymin<y2 and y2<ymax) or
   ...
   -- print(xmax-x0,ymax-y0-0.05)
    ...
    if horizontal_implicit_rational_quadratic_test(x0,y0,x1,y1,w1,x2,y2,signh,xmin+0.05,ymin) == true and
      horizontal_implicit_rational_quadratic_test(x0,y0,x1,y1,w1,x2,y2,signh,xmax-0.05,ymin) == false and
      symin <= ymin and ymin <= symax then
          return true
    end
    if horizontal_implicit_rational_quadratic_test(x0,y0,x1,y1,w1,x2,y2,signh,xmin+0.05,ymax) == true and
      horizontal_implicit_rational_quadratic_test(x0,y0,x1,y1,w1,x2,y2,signh,xmax-0.05,ymax) == false and
      symin <= ymax and ymax <= symax then
          return true
    end
  end
