...
static int _optindex(lua_State *L, int arg, int top, int def) {
	int idx = static_cast<int>((def ? luaL_optinteger(L, arg, def) : luaL_checkinteger(L, arg)));
...
static int luaVA_tuple(lua_State *L) {
	int n = static_cast<int>(lua_tointeger(L, lua_upvalueindex(1)));  /* number of packed values */
...
#endif
int luaopen_vararg(lua_State *L) {
    lua_newtable(L);
	compat_setfuncs(L, va_funcs, 0);
	return 1;
}
