...
local function build_face_to_filename(files)
    ...
    for i,filename in pairs(files) do
        ...
        local style = splitwords(freetypeface.style_name)
        permute(style, function(u)
            local name = string.lower(freetypeface.face_family .. " " ..
                table.concat(u, " "))
...
function _M.loadface(family_and_styles_or_filename)
    ...
    local freetypeface = assert(freetype.face(filename))
    local harfbuzzfont = assert(harfbuzz.font(filename))
    harfbuzzfont:setscale(freetypeface.units_per_EM)
    harfbuzzfont:setfuncs()
    f = {
